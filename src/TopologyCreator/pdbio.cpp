...
void readPDB(const char* filename, PDB* pdbData){
	...
	if ( file != NULL ){
		...
		while(fgets(buffer, BUF_SIZE, file) != NULL){
			...
			if(strcmp(pch, "ATOM") == 0 || strncmp(buffer, "HETATM", 6) == 0){
...
void parseAtomLine(PDB* pdbData, char* line, int currentAtom){
	...
	if(strncmp(line, "HETATM", 6) == 0){
		...
		pdbData->atoms[currentAtom].isHETATM = 0;
	}
#ifdef DEBUGPDBIO
	...
	if(printConnections){
		for(i = 0; i < pdbData->atomCount; i++){
			...
			for(j = 0; j < pdbData->connections.connectCount[i]; j++){
				fprintf(file, "CONECT%5d%5d\n", i+1,pdbData->connections.connectMap[j*pdbData->atomCount + i]+1);
			}
			//fprintf(file, "\n");
		}
	}
